function [Re_hMtr,Im_hMtr, loss2] = forward_update_SIMO_ISI_Channel_4QAM_L3(ResigVec, ImsigVec, labelVec, beta, lr, batch_size,testBits)
...
dLdReh = zeros(N,3,batch_size);
dLdImh = zeros(N,3,batch_size);
Re_hMtr = zeros(N,3);
Im_hMtr = zeros(N,3);
...
loss2 = zeros(1,epoch);
% loss4 = zeros(1,epoch);
vtRe = zeros(N,3); %initial gradient
vtIm = zeros(N,3);
...
for idx = 1 : epoch
    for jj = 1 : iternum
        ...
        for ii = 1 : batch_size
            ...
            [dLdReh(:,:,ii),dLdImh(:,:,ii),loss(ii)] = GenGradient_SIMO_ISI_Channel_L3(ResigVec( ii + (jj-1)*batch_size , : ),ImsigVec( ii + (jj-1)*batch_size , : ),Re_hMtr,Im_hMtr,labelVec( ii + (jj-1)*batch_size), xkMtr);            
