function [DecodeBit,DecodeSym]  = Viterbi_ISI_L2_4QAM_inf_L3( rcvSig,chanH)
...
% PathMetric(1) = 0;
% PathMetric(2) = 0;
numBits = length(rcvSig);
% % test
trellisOutput = state_tran_output_4QAM_L3(chanH);
BranchMetric = zeros( size(trellisOutput) );
for i = 1 : numBits
    NewPathMetric = zeros(16,1);
    % BranchMetric  = distance_branch_one_bit_L2( zk(i),channelH,noiseVar); % distance -log(p)
    % BranchMetric = distance_branch_4QAM( zk(i),channelH,noiseVar);
    symbol = rcvSig(i);
    for m = 1 : size(trellisOutput,1)
        for n = 1 : size(trellisOutput,2)
            BranchMetric(m,n) = norm(trellisOutput(m,n)-symbol,2).^2;
        end
    end
    
    % BranchMetric  = distance_branch( branch,symbol); % distance -log(p)
    %% viterbi decode
    % update P1
