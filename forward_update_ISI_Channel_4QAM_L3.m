function [Re_h,Im_h,loss2] = forward_update_ISI_Channel_4QAM_L3(ResigVec, ImsigVec, labelVec, beta, lr, batch_size,testBits)
...
loss2 = zeros(1,epoch);
% loss4 = zeros(1,epoch);
vtRe = zeros(1,3); %initial gradient
vtIm = zeros(1,3);
% treliss output
...
for idx = 1 : epoch
    for jj = 1 : iternum
        dLdReh = zeros(batch_size,3);
        dLdImh = zeros(batch_size,3);
        ...
        for ii = 1 : batch_size
            ...
            % [dLdReh(ii,:),dLdImh(ii,:),loss(ii)] = GenGradient_ISI_Channel(ResigVec( ii + (jj-1)*batch_size ),ImsigVec( ii + (jj-1)*batch_size ),Re_h,Im_h,labelVec( ii + (jj-1)*batch_size), xkMtr);
              ...
              [dLdReh(ii,:),dLdImh(ii,:),loss(ii)] = GenGradient_ISI_Channel_4QAM_L3(ResigVec( ii + (jj-1)*batch_size ),ImsigVec( ii + (jj-1)*batch_size ),Re_h,Im_h,labelVec( ii + (jj-1)*batch_size), xkMtr);
