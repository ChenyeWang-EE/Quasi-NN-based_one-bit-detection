function [DecodeBit,DecodeSym] = QNN_viterbi_one_bit_detection_4QAM_L3(zk, Re_h,Im_h)
%% upsample = 1;
% len = length(zk);
% yprob = zeros(8,8,len);
...
numBits = length(zk);
% % test
distanceMtr = QNN_distance_branch_4QAM_L3( zk,Re_h,Im_h); % numBits × 16 Matrix
for i = 1 : numBits
    ...
    BranchMetric  = reshape( distanceMtr(i,:),4,16).' ; % distance -log(p)
%% viterbi decode
    % update P1
